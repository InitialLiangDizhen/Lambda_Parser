...
instance Functor ParseResult where  --Result i take i as Input string
  fmap f (Result i a) = Result  i $ f a -- f is function, a is parser
  fmap _ (Error e)    = Error e         --Result i $ f . a
                                        --(Result i $ f a = Result i' $ f a 
...
instance Monad Parser where
  fa >>= f = P (\i ->
    case parse fa i of     -- fa is the parser on i, successful give Result
      Result i' a -> parse (f a) i'  --parse (f a) parse $ apply f on a
      Error e     -> Error e)
...
satisfy f = do
  c <- get  -- Consumes and returns the next character. Fails if there is no input left.
